...
export default class StorageMechanism {
  ...
  init() {
    ...
    open.onsuccess = (event) => {
      ...
      this.sessions_ =
          StorageMechanism.createEmeSession_(db)
      p.resolve()
    }
   ...
   */
  getEmeSessionCell() {
    console.assert(this.sessions_, 'Cannot be destroyed.')
  ...
  static createV1_(db) {
    ...
    if (stores.contains(manifestStore) && stores.contains(segmentStore)) {
      console.debug('Mounting v1 idb storage cell')

      return new V1StorageCell(
        db,
        segmentStore,
        manifestStore)
    }
  ...
  static createV2_(db) {
    ...
    const stores = db.objectStoreNames
    if (stores.contains(manifestStore) && stores.contains(segmentStore)) {
      console.debug('Mounting v2 idb storage cell')

      return new V2StorageCell(
        db,
        segmentStore,
        manifestStore,
  ...
  static createV3_(db) {
    ...
    const stores = db.objectStoreNames
    if (stores.contains(manifestStore) && stores.contains(segmentStore)) {
      console.debug('Mounting v3 idb storage cell')
  ...
  static createEmeSession_(db) {
    ...
    if (db.objectStoreNames.contains(store)) {
      console.debug('Mounting session ID idb storage cell')
