...
export default class Storage {
  ...
  store(uri, appMetadata, mimeType) {
    const getParser = async() => {
      ...
      return Functional.callFactory(factory)
    }
  ...
  async parseManifest(uri, getParser) {
    ...
    try {
      ...
      const streams =
          Storage.getAllStreamsFromManifest_(manifest)
  ...
  createPeriod_(downloader, storage, estimator, drmEngine, manifest, period) {
    ...
    // for each of them.
    const streamSet = Storage.getAllStreamsFromPeriod_(period)
  ...
  static validatePeriod_(period) {
    ...
    if (videos.size > 1) {
      console.warning('Multiple video tracks selected to be stored')
    }
...
Storage.defaultSystemIds_ = new Map()
