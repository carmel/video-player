...
export class NetworkingEngine extends FakeEventTarget {
  ...
  request(type, request) {
    ...
    const op = responseFilterOperation.chain((responseAndGotProgress) => {
      ...
      // retries.
      if (e) {
        console.assert(e instanceof Error, 'Wrong error type')
        e.severity = Error.Severity.CRITICAL
      }
  ...
  filterRequest_(type, request) {
    ...
    return filterOperation.chain(undefined, (e) => {
      ...
      throw new Error(
        ...
        Error.Code.REQUEST_FILTER_ERROR, e)
    })
  }
  ...
  filterResponse_(type, responseAndGotProgress) {
    ...
    return filterOperation.chain(() => {
      ...
      throw new Error(
        severity,
        Error.Category.NETWORK,
        Error.Code.RESPONSE_FILTER_ERROR, e)
    })
  }
