...
export class EbmlParser {
  ...
  parseVint_() {
    ...
    console.assert(numBytes <= 8 && numBytes >= 1, 'Incorrect log2 value')
  ...
  static getVintValue_(vint) {
    ...
    let value = 0
    for (const { item, i } of Iterables.enumerate(vint)) {
      if (i === 0) {
...
export class EbmlElement {
  ...
  getUint() {
    ...
    for (const i of Iterables.range(this.dataView_.byteLength)) {
