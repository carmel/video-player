...
export default class DashParser {
  ...
  constructor() {
     ...
     * e.g.: '1,5,23'
     * @private {!Object.<string, !SegmentIndex>}
     */
   ...
   */
  configure(config) {
    console.assert(config.dash !== null,
  ...
  async update() {
    try {
      ...
      console.assert(error instanceof Error, 'Bad error type')
  ...
  parsePeriods_(context, baseUris, mpd) {
    ...
    for (const { i, item: elem, next } of enumerate(periodNodes)) {
      ...
      if (periodDuration && givenDuration &&
        ...
        console.warning('There is a gap/overlap between Periods', elem)
      }
      ...
      console.assert(periodId, 'Period IDs should not be null!')
  ...
  createVariants_(audio, video, variants) {
    ...
    console.assert(!audio || audio.contentType === ContentType.AUDIO,
  ...
  parseAdaptationSet_(context, elem) {
    ...
    if (isImage) {
      console.warning('Skipping Image AdaptationSet', context.adaptationSet)
      return null
    }
  ...
  parseRepresentation_(context, kind, language, label,
    ...
    if (!this.verifyRepresentation_(context.representation)) {
      console.warning('Skipping Representation', context.representation)
      return null
    }
   ...
   */
  async onUpdate_() {
    console.assert(this.updatePeriod_ >= 0,
  ...
  createFrame_(elem, parent, baseUris) {
    console.assert(parent || baseUris,
      ...
      switch (scheme) {
        ...
        case 'urn:dts:dash:audio_channel_configuration:2012': {
          ...
          if (!intValue) { // 0 or NaN
            console.warning('Channel parsing failure! ' +
  ...
  verifyRepresentation_(frame) {
    ...
    if (n !== 1) {
      ...
      if (frame.segmentBase) {
        console.info('Using SegmentBase by default.')
        frame.segmentList = null
        frame.segmentTemplate = null
      } else {
        console.assert(frame.segmentList, 'There should be a SegmentList')
        console.info('Using SegmentList by default.')
        frame.segmentTemplate = null
      }
    }
  ...
  async requestForTiming_(baseUris, uri, method) {
    ...
    if (method === 'HEAD') {
      if (!response.headers || !response.headers['date']) {
        console.warning('UTC timing response is missing',
    ...
    if (isNaN(date)) {
      console.warning('Unable to parse date from UTC timing response')
      return 0
    }
      ...
      try {
        ...
        switch (scheme) {
          ...
          case 'urn:mpeg:dash:utc:sntp:2014':
            console.alwaysWarn('NTP UTCTiming scheme is not supported')
            break
          default:
            console.alwaysWarn(
        ...
        console.warning('Error fetching time from UTCTiming elem', e.message)
      }
    }
 ...
 * @property {!PresentationTimeline} presentationTimeline
 ...
 * methods.
 *
 * @property {DashParser.GenerateSegmentIndexFunction}
 *     generateSegmentIndex
