...
import { PreferenceBasedCriteria, ExampleBasedCriteria } from './media/adaptation_set_criteria'
 ...
 * @property {string} type
 *   'emsg'
 * @property {extern.EmsgInfo} detail
 *   An object which contains the content of the emsg box.
 * @exportDoc
 */

/**
 * @event DrmSessionUpdateEvent
...
/**
 * @event TimelineRegionAddedEvent
 * @description Fired when a media timeline region is added.
 * @property {string} type
 *   'timelineregionadded'
 * @property {extern.TimelineRegionInfo} detail
 *   An object which contains a description of the region.
 * @exportDoc
 */

/**
 * @event TimelineRegionEnterEvent
 ...
 */
/**
 * @event TextTrackVisibilityEvent
...
export default class Player extends FakeEventTarget {
  ...
  constructor(mediaElement) {
     ...
     *
     * @private {PlayheadObserverManager}
     */
    ...
    /** @private {extern.ManifestParser} */
    this.parser_ = null

    /** @private {?extern.ManifestParser.Factory} */
    this.parserFactory_ = null

    /** @private {?extern.Manifest} */
    this.manifest_ = null

    /** @private {?string} */
    this.assetUri_ = null

    /** @private {extern.AbrManager} */
    this.abrManager_ = null

    /**
     * The factory that was used to create the abrManager_ instance.
     * @private {?extern.AbrManager.Factory}
     */
    ...
    /** @private {?extern.PlayerConfiguration} */
     ...
     * @private {?extern.TextDisplayer.Factory}
     */
    ...
    /** @private {extern.IAdManager} */
    this.adManager_ = null

    if (adManagerFactory_) {
      this.adManager_ =
          Functional.callFactory(adManagerFactory_)
    }
    ...
    const walkerImplementation = {
      ...
      handleError: async(has, error) => {
        console.warning('The walker saw an error:')
        if (error instanceof Error) {
          console.warning('Error Code:', error.code)
        } else {
          console.warning('Error Message:', error.message)
          console.warning('Error Stack:', error.stack)
        }
   ...
   *
   * @param {!extern.IAdManager.Factory} factory
   * @export
   */
  static setAdManagerFactory(factory) {
    this.adManagerFactory_ = factory
  }
   ...
   * @return {!Promise.<extern.SupportType>}
   * @export
   */
  static async probeSupport() {
    console.assert(Player.isBrowserSupported(),
  ...
  unload(initializeMediaSource = true) {
    ...
    const events = this.walker_.startNewRoute((currentPayload) => {
      ...
      console.assert(destination, 'We should have picked a destination.')
    ...
    events.onStart = () => console.info('Starting unload...')
    return this.wrapWalkerListenersWithPromise_(events)
  }
  ...
  load(assetUri, startTime, mimeType) {
    ...
    // error.
    events.onStart =
        () => console.info('Starting load of ' + assetUri + '...')
  ...
  shouldUseSrcEquals_(payload) {
    ...
    if (mimeType) {
      ...
      const canPlayMediaSource =
          ManifestParser.isSupported(uri, mimeType)
      ...
      console.assert(canPlayNatively && canPlayMediaSource,
  ...
  async onUnload_(has, wants) {
    ...
    if (has.mediaElement && has.mediaElement.src) {
      ...
      // otherwise.
      await new Promise(
        (resolve) => new Timer(resolve).tickAfter(0.1))
  ...
  async onInitializeMediaSourceEngine_(has, wants) {
    ...
    const closedCaptionsParser =
        MuxJSClosedCaptionParser.isSupported()
    ...
    const textDisplayer =
        Functional.callFactory(textDisplayerFactory)
  ...
  async onInitializeParser_(has, wants) {
    ...
    const manifestConfig =
        ObjectUtils.cloneObject(this.config_.manifest)
  ...
  async onLoad_(has, wants) {
    ...
    if (!this.config_.preferredAudioLanguage && !hasPrimary) {
      console.warning('No preferred audio language set.  We will choose an ' +
  ...
  static filterForAVVariants_(periods) {
    ...
    if (hasAVVariant) {
      console.debug('Found variant with audio and video content, ' +
  ...
  chooseCodecsAndFilterManifest_() {
    ...
    variantsByCodecs.forEach((codecs, variants) => {
      ...
      console.debug('codecs', codecs, 'avg bandwidth', averageBandwidth)
    ...
    console.assert(bestCodecs !== null, 'Should have chosen codecs!')
    console.assert(!isNaN(lowestAverageBandwidth),
    ...
    for (const period of this.manifest_.periods) {
      period.variants = period.variants.filter((variant) => {
        ...
        console.debug('Dropping Variant (better codec available)', variant)
        return false
      })
    }
  }
  ...
  updateBufferingSettings_(rebufferingGoal) {
    ...
    const satisfiedThreshold = Math.min(
      TYPICAL_BUFFERING_THRESHOLD_, rebufferingGoal / 2)
  ...
  configure(config, value) {
    ...
    console.assert(typeof (config) === 'object' || arguments.length === 2,
  ...
  applyConfig_() {
    ...
    if (this.streamingEngine_) {
      ...
      const activeVariant = StreamUtils.getVariantByStreams(
      ...
      if (this.abrManager_ && activeVariant &&
        ...
        console.debug('Choosing new streams after changing configuration')
   ...
   */
  resetConfiguration() {
    console.assert(this.config_, 'Cannot be destroyed')
  ...
  /**
   * @return {NetworkingEngine} A reference to the Player's networking
  ...
  /**
    * Returns a ads.AdManager instance, responsible for Dynamic
    * Ad Insertion functionality.
    *
    * @return {extern.IAdManager}
    * @export
    */
  getAdManager() {
  ...
  trickPlay(rate) {
    ...
    console.assert(rate !== 0, 'Should never set a trick play rate of 0!')
    if (rate === 0) {
      console.warn('A trick play rate of 0 is unsupported!')
      return
    }
  ...
  getVariantTracks() {
    if (this.manifest_ && this.playhead_) {
      ...
      return audioTracks.map((audio) =>
        StreamUtils.html5AudioTrackToTrack(audio))
    } else {
      return []
    }
  }
  ...
  selectTextTrack(track) {
    if (this.manifest_ && this.streamingEngine_) {
      ...
      if (!stream) {
        console.error('No stream with id', track.id)
        return
      }
  ...
  selectVariantTrack(track, clearBuffer, safeMargin = 0) {
    if (this.manifest_ && this.streamingEngine_) {
      ...
      if (this.config_.abr.enabled) {
        console.warn('Changing tracks while abr manager is enabled ' +
      ...
      if (!variant) {
        console.error('No variant with id', track.id)
        return
      }
      ...
      if (!StreamUtils.isPlayable(variant)) {
        console.error('Unable to switch to restricted track', track.id)
        return
      }
      ...
      this.currentAdaptationSetCriteria_ = new ExampleBasedCriteria(
        variant)
   ...
   * list.
   *
   * @return {!Array.<extern.LanguageRole>}
   * @export
   */
  ...
  selectAudioLanguage(language, role) {
    if (this.manifest_ && this.playhead_) {
      ...
      this.currentAdaptationSetCriteria_ =
          new PreferenceBasedCriteria(language, role || '',
  ...
  selectVariantsByLabel(label) {
    if (this.manifest_ && this.playhead_) {
      ...
      if (firstVariantWithLabel === null) {
        console.warning('No variants were found with label: ' +
  ...
  getPlayheadTimeAsDate() {
    if (!this.isLive()) {
      console.warning('getPlayheadTimeAsDate is for live streams!')
      return null
    }
    ...
    if (this.manifest_) {
      ...
      console.warning('No way to get playhead time as Date!')
      return null
    }
  }
  ...
  getPresentationStartTimeAsDate() {
    if (!this.isLive()) {
      console.warning('getPresentationStartTimeAsDate is for live streams!')
      return null
    }
    ...
    if (this.manifest_) {
      ...
      console.warning('No way to get presentation start time as Date!')
      return null
    }
  }
  ...
  getStats() {
    ...
    console.assert(this.video_, 'If we have stats, we should have video_')
    ...
    this.stats_.setLicenseTime(NaN)
  ...
  async addTextTrack(uri, language, kind, mime, codec, label) {
    ...
    console.assert(period, 'The period should still be non-null here.')
    ...
    console.debug('Choosing new streams after adding a text stream')
    this.chooseStreamsAndSwitch_(period)
    this.onTracksChanged_()
   ...
   * This will be called automatically by <code>cast.CastReceiver</code>
   ...
   *
   * @return {?extern.ManifestParser.Factory}
   * @export
   */
  ...
  defaultConfig_() {
    ...
    config.textDisplayFactory =
        () => new SimpleTextDisplayer(this.video_)
  ...
  assertCorrectActiveStreams_() {
    if (!this.streamingEngine_ || !this.manifest_ || !conf.DEBUG) {
      return
    }
  ...
  onRateChange_() {
    ...
    const event = this.makeEvent_(EventName.RateChange)
    this.dispatchEvent(event)
  }
  ...
  switch_(variant, clearBuffer = false, safeMargin = 0) {
    ...
    console.assert(this.config_.abr.enabled,
      'AbrManager should not call switch while disabled!')
    console.assert(!this.switchingPeriods_,
      'AbrManager should not call switch while transitioning between ' +
        'Periods!')
    console.assert(this.manifest_, 'We need a manifest to switch ' +
                                        'variants.')
  ...
  onKeyStatus_(keyStatusMap) {
    ...
    if (activeVariant && !activeVariant.allowedByKeySystem) {
      console.debug('Choosing new variants after key status changed')
  ...
  onExpirationUpdated_(keyId, expiration) {
    ...
    const event = this.makeEvent_(EventName.ExpirationUpdated)
    this.dispatchEvent(event)
  }
   ...
   *
   * @param {!Array.<?extern.Track>} tracks
   * @return {!Set.<string>}
   * @private
   */
  ...
  getPresentationPeriod_() {
    console.assert(this.manifest_ && this.playhead_,
  ...
  getPresentationText_() {
    ...
    if (!this.activeStreams_.getText(currentPeriod)) {
      const textStreams = StreamUtils.filterStreamsByLanguageAndRole(
  ...
  isBufferedToEndMS_() {
    ...
    if (this.manifest_.presentationTimeline.isLive()) {
      ...
      const bufferEnd =
          TimeRangesUtils.bufferEnd(this.video_.buffered)
 ...
 * @export
 */
const version = 'v2.5.9-master-uncompiled'
...
const supportPlugins_ = {}
/** @private {?extern.IAdManager.Factory} */
const adManagerFactory_ = null
